...
// NoPodConflictSelectorsMatch evaluates if a pod fits a node by:
// 1) Comparing the pod's conflict selectors against labels of all existing pods.
// 2) Comparing existing pods conflict selectors with labels of this pod.
func NoPodConflictSelectorsMatch(pod *api.Pod, existingPods []*api.Pod, node string) (bool, error) {
	for podIx := range existingPods {
		if existingPods[podIx].Labels != nil {
			for ii, _ := range pod.Spec.PodConflictSelectors {
				if pod.Spec.PodConflictSelectors[ii].Matches(labels.Set(existingPods[podIx].Labels)) {
					return false, nil
				}
			}
		}
		if pod.Labels != nil {
			for ii, _ := range existingPods[podIx].Spec.PodConflictSelectors {
				if existingPods[podIx].Spec.PodConflictSelectors[ii].Matches(labels.Set(pod.Labels)) {
					return false, nil
				}
			}
		}
